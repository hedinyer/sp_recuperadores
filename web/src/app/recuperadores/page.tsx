...
export default function RecuperadoresPage() {
  ...
  const [mensajeRecibo, setMensajeRecibo] = useState<string | null>(null);
  ...
  const placasConDeuda = useMemo(
    () =>
      [...new Set(asignacionesPendientes.map((a) => a.placa))]
        .sort()
        .join(","),
  ...
  const compartirReciboWpp = useCallback(async () => {
    ...
    try {
      ...
      setMensajeRecibo("No se pudo generar la imagen. Se abrió WhatsApp solo con texto.");
      abrirWhatsAppConTexto(texto);
    } finally {
  ...
  const descargarRecibo = useCallback(async () => {
    ...
    try {
      ...
      setMensajeRecibo("No se pudo descargar el recibo. Intenta de nuevo.");
    } finally {
      setExportandoRecibo(false);
    }
  }, [recibo, exportandoRecibo]);
    ...
    <div className="min-h-dvh flex flex-col bg-zinc-950 text-zinc-100 pt-[max(0.75rem,env(safe-area-inset-top))]">
      ...
      <main className="flex-1 w-full max-w-[414px] mx-auto px-3 sm:px-4 pt-3 flex flex-col gap-3 min-h-0">
        ...
        {mensajeRecibo && (
          <div
            role="status"
            className="shrink-0 rounded-xl border border-blue-900/60 bg-blue-950/40 px-3.5 py-2.5 text-sm text-blue-200"
          >
            {mensajeRecibo}
          </div>
        )}
            ...
            {loading ? (
              ...
              >
                ...
                {asignacionesPendientes.map((asig) => (
                  <article
                    key={asig.id}
                    className="rounded-2xl border px-4 py-3 border-zinc-800 bg-zinc-900/60"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <span className="ml-2 text-[10px] uppercase px-1.5 py-0.5 rounded-full bg-amber-900/60 text-amber-300">
                          {asig.estado}
                        </span>
                        {asig.gps_moto && (
                      ...
                      <span className="text-[10px] text-zinc-500 tabular-nums text-right">
